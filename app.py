...
# --- ADD NEW HELPER FUNCTION FOR VISUALIZATION ON VIDEO ---

def visualize_predictions_on_video(input_video_path, output_video_path, model_svm, label_encoder, movenet_model, expand_keypoints):
    ...
    while cap.isOpened():
        ...
        results = movenet_model.signatures['serving_default'](input_tensor)
        keypoints_list = extract_keypoints(results)
        if not keypoints_list:
            out_writer.write(frame)
            continue

        # Predict punches for each detected person
        for kp in keypoints_list:
            features = expand_keypoints(kp)
            features = pd.DataFrame([features]).fillna(0)
            X_sample = features.values
            pred_encoded = model_svm.predict(X_sample)[0]
            pred_label = label_encoder.inverse_transform([pred_encoded])[0]

            # Draw predicted punch on frame near keypoints
            h, w = frame.shape[:2]
            visible_points = [(y, x) for (y, x, s) in kp if s > 0.2]
            if visible_points:
                y_coords, x_coords = zip(*visible_points)
                min_x = int(min(x_coords) * w)
                max_y = int(max(y_coords) * h)
                cv2.putText(frame, f"Punch: {pred_label}", (min_x, max_y + 40),
...
st.title("Boxing Pose Estimation & Punch Detection")

uploaded_file = st.file_uploader("Upload boxing video", type=["mp4", "mov"])
...
if uploaded_file is not None:
    ...
    # Your existing pose estimation, skeleton drawing, punch detection code here...
    # This includes extracting keypoints per frame, drawing skeletons, punch classification logs etc.
    # Assume you accumulate punch data logs in `all_logs` list/dict with keypoints and punch labels.

    # Example structure:
    # all_logs = [
    #   {'keypoints': [...], 'punch': 'jab', ...},
    #   {'keypoints': [...], 'punch': 'cross', ...},
    #    ...
    # ]
    ...
    if st.button(f"Train SVM on {uploaded_file.name}"):
        if 'all_logs' in locals() and all_logs:
            df_all = pd.DataFrame(all_logs)

            # Expand keypoints into columns x_0, y_0, s_0, x_1, y_1, s_1, ...
            expanded = df_all['keypoints'].apply(expand_keypoints)
            df_full = pd.concat([df_all.drop(columns=['keypoints']), expanded], axis=1).fillna(0)

            # Filter feature columns (x_, y_, s_)
            feature_cols = [col for col in df_full.columns if col.startswith(('x_', 'y_', 's_'))]
            ...
            model_path = os.path.join(temp_dir, f"{os.path.splitext(uploaded_file.name)[0]}_svm_model.joblib")
            joblib.dump({'model': model_svm, 'label_encoder': le}, model_path)
            st.download_button("📥 Download SVM Model", data=open(model_path, "rb"), file_name=os.path.basename(model_path))

            st.success(f"✅ SVM trained and saved for {uploaded_file.name}")

            # Visualize predictions on video frames
            pred_vis_path = os.path.join(temp_dir, f"predicted_{uploaded_file.name}")
            # You need to pass your MoveNet model and expand_keypoints function here:
            visualize_predictions_on_video(input_path, pred_vis_path, model_svm, le, movenet_model, expand_keypoints)

            st.video(pred_vis_path)
            st.success(f"✅ Prediction visualization ready for {uploaded_file.name}")

        else:
            st.warning("No punch data logs available for training!")
