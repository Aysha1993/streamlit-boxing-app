...
def extract_valid_keypoints(keypoints_with_scores, threshold=0.2):
    ...
    for person in keypoints_with_scores:
        if person[55] < threshold:  # person confidence
            continue
...
def analyze_video(video_path, output_path="output_annotated.mp4"):
    ...
    return "punch_log.csv", output_path

# Streamlit App UI
st.title("🥊 Boxing Analyzer App")
uploaded_file = st.file_uploader("Upload a boxing video (MP4)", type=["mp4"])
...
if uploaded_file is not None:
    ...
    if st.button("Run Analyzer"):
        punch_csv, annotated_path = analyze_video(tmp_path)
        st.success("Analysis complete ✅")

        st.video(annotated_path)

        with open(punch_csv, "rb") as f:
            st.download_button("Download Punch Log CSV", f, file_name="punch_log.csv")

        df = pd.read_csv(punch_csv)
        st.dataframe(df)

        if st.button("Train SVM Model on Punch Data"):
            if "punch" in df.columns:
                df['label'] = df['punch'].astype('category').cat.codes
                X = df[['frame', 'person']]
                y = df['label']
                clf = svm.SVC()
                clf.fit(X, y)
                model_dest = "svm_punch_model.joblib"
                dump(clf, model_dest)
                st.success(f"SVM Model trained and saved as {model_dest}")
